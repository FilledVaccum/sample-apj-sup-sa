...
| **variables-compare** | Blue(3.04)/Green(3.10) 의 `SHOW VARIABLES` 를 비교해 추가/삭제/변경된 값을 정리 (default vs 커스터마이즈 구분) | 양쪽 `SHOW VARIABLES` |
| **error-log-analyzer** | Green 클러스터의 에러 로그에서 3.04 → 3.10 마이너 업그레이드 관련 에러·경고·deprecation 추출 | CloudWatch Logs (`GREEN_LOG_GROUP`) |
| **upgrade-readiness** | InnoDB 엔진 상태 분석 + 업그레이드 전 옵티마이저 패턴 리스크 식별 | Blue 의 `SHOW ENGINE INNODB STATUS` + `performance_schema` |
| **query-risk-scorer** | `performance_schema` digest 별로 업그레이드 위험도 점수화(디스크 임시테이블, 풀스캔, 정렬량, no-good-index) 후 옵티마이저 민감 SQL 패턴별 분류 | Blue 의 `performance_schema` digest |
| **plan-diff** | 최근 SELECT 들을 Blue/Green 양쪽에서 `EXPLAIN FORMAT=JSON` 실행해 plan 이 실제로 바뀌는 쿼리만 보고 | 양쪽 `EXPLAIN` (Blue 의 `events_statements_history` 기반) |
