...
| **variables-compare** | Compares `SHOW VARIABLES` between Blue (8.0) and Green (8.4) and flags added / removed / changed values (default vs customized) | `SHOW VARIABLES` on both |
| **error-log-analyzer** | Scans the Green instance's MySQL error log for errors, warnings, and deprecations relevant to the 8.0 → 8.4 upgrade | CloudWatch Logs (`GREEN_LOG_GROUP`) |
| **upgrade-readiness** | Analyzes InnoDB engine status and scores queries whose optimizer plans are most likely to shift on 8.4 | `SHOW ENGINE INNODB STATUS` + `performance_schema` on Blue |
...
- **An active Blue-Green Deployment exists.** The very first step
  ...
  instances; if either connection fails, the workflow stops immediately. A
  standalone instance with no Green is not enough.
...
- **The DB user can read diagnostics.** The default `admin` (or your `DB_USER`)
  needs `SHOW VARIABLES`, `SHOW ENGINE INNODB STATUS`, and `SELECT` on
  `performance_schema`.
...
> (8.0) and the upgraded (8.4) instances side by side, so the agents can read
